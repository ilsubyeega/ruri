...
namespace PacketBuilder::CT::BotInfo {
	...
	constexpr const auto Bot_Panel = PopulateHeader(Concate(
		...
		Number<int>(119), String(M_BOT_NAME), Number<byte>(0), Number<byte>(BOT_LOCATION), Number<byte>(UserType::Peppy),
	...
	constexpr const auto Bot_Stats = PopulateHeader(
		Concate(
			...
			Number<int>(119), Number<byte>(0), Number<byte>(0), Number<byte>(0), Number<int>(0), Number<byte>(0), Number<int>(0), Number<long long>(0),
...
void receiveConnections(){
	...
	printf("Creating LazyThread\n");
	{
		std::thread a(LazyThread);
		a.detach();
	}
	printf("Creating Aria Thread\n");
	{
		std::thread a(Aria_Main);
		a.detach();
	}
	printf("Creating API thread\n");
	{
